...
public class ProdutoDAO {
    ...
    public static List<Produto> getProdutos(Context context){
        ...
        if( cursor.getCount() > 0 ){
            cursor.moveToFirst();
            do{
                Produto prod = new Produto("Empty list/Lista vazia...", "");
                prod.setId(  cursor.getInt( 0 ) );
                prod.setNome( cursor.getString( 1 ));
                prod.setQuantidade( cursor.getDouble( 2 ));
                prod.setCategoria( cursor.getString( 3 ));
    ...
    public static Produto getProdutoById(Context context, int idProduto){
        ...
        if( cursor.getCount() > 0 ){
            ...
            Produto prod = new Produto("lista vazia", "");
            prod.setId(  cursor.getInt( 0 ) );
            prod.setNome( cursor.getString( 1 ));
            prod.setQuantidade( cursor.getDouble( 2 ));
            prod.setCategoria( cursor.getString( 3 ));
